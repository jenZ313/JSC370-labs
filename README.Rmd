...
met[, temp_dist := abs(temp - medians$temp_50)]
median_temp_station <- met[order(temp_dist)][1]

met[, wind.sp_dist := abs(wind.sp - medians$wind.sp_50)]
median_wind.sp_station <- met[order(wind.sp_dist)][1]

met[, atm.press_dist := abs(atm.press - medians$atm.press_50)]
median_atm.press_station <- met[order(atm.press_dist)][1]

median_temp_station[, .(USAFID, STATE, temp, temp_dist)]
median_wind.sp_station[, .(USAFID, STATE, wind.sp, wind.sp_dist)]
median_atm.press_station[, .(USAFID, STATE, atm.press, atm.press_dist)]
```

Station 720113 is closest to the median temperature, while station 690150 is closest to both the median wind speed and atmospheric pressure, so the stations do not fully coincide.
...
```{r}
medians_by_state <- met[, .(
  temp_50 = quantile(temp, probs = 0.5, na.rm = TRUE),
  wind.sp_50 = quantile(wind.sp, probs = 0.5, na.rm = TRUE)
), by = STATE]

met <- merge(met, medians_by_state, by = "STATE", all.x = TRUE)
met[, eudist := sqrt((temp - temp_50)^2 + (wind.sp - wind.sp_50)^2)]


most_representative_stations <- met[
  , .SD[which.min(eudist)][order(lat)][1], 
  by = STATE
]


result <- most_representative_stations[, .(
  STATE,
  USAFID,
  temp,
  wind.sp,
...
met <- merge(met, state_midpoints, by = "STATE", all.x = TRUE)
met[, dist_to_mid := sqrt((lat - mid_lat)^2 + (lon - mid_lon)^2)]
closest_to_midpoint <- met[, .SD[which.min(dist_to_mid)], by = STATE]


combined_stations <- rbindlist(list(
  most_representative_stations[, .(STATE, USAFID, lat, lon, type = "representative")],
  closest_to_midpoint[, .(STATE, USAFID, lat, lon, type = "midpoint")]
), use.names = TRUE)
...
leaflet(data = combined_stations) %>%
  ...
  addCircleMarkers(
    lng = ~lon,
    lat = ~lat,
    color = ~ifelse(type == "representative", "coral", "lightblue"),
    label = ~paste("USAFID:", USAFID, "<br>Type:", type),
    radius = 5
  )
...
state_avg_temp[, temp_level := fifelse(
  avg_temp < 20, "Low",
...
summary_table <- state_avg_temp[, .(
  ...
  avg_wind_sp = mean(avg_wind_sp, na.rm = TRUE),
  avg_atm_press = mean(avg_atm_press, na.rm = TRUE)
), by = temp_level]
...
station_med_lt <- lazy_dt(station_med) |>
  filter(between(avg_atm_press, 1000, 1020)) |>
  collect()

ggplot(station_med_lt, aes(x = avg_atm_press, y = avg_temp)) +
  geom_point() +
  geom_smooth(method = "lm", col = "coral") +
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cr"), col = "lightblue")

lm_mod <- lm(avg_temp ~ avg_atm_press, data = station_med_lt)
summary(lm_mod)

gam_mod <- gam(avg_temp ~ s(avg_atm_press, bs = "cr", k = 20), data = station_med_lt)
summary(gam_mod)
plot(gam_mod)
...
The linear model shows a strong negative relationship between atmospheric pressure and temperature but only explains 20.7% of the variation. The GAM model fits better because it captures the non-linear pattern and explains more variation (27.6%), making it the better choice.